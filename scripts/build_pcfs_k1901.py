...
def main() -> int:
    ...
    tags_file = PROJECT_ROOT / "config" / "tags_k19_01.txt"
    out_parquet = PROJECT_ROOT / "data" / "processed" / "K-19-01_1y_0p1h.parquet"

    plant = "PCFS"
    unit = "K-19-01"
    server = r"\\PTSG-1MMPDPdb01"  # change if PCFS uses a different PI server
    start = "-1y"
    end = "*"
    step = "-0.1h"
    work_sheet = "DL_WORK"
