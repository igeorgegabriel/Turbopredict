...
def main() -> int:
    ...
    tags_file = PROJECT_ROOT / "config" / "tags_pcmsb_c13001.txt"
    out_parquet = PROJECT_ROOT / "data" / "processed" / "C-13001_1y_0p1h.parquet"

    plant = "PCMSB"
    unit = "C-13001"
    server = r"\\PTSG-1MMPDPdb01"  # change if PCMSB uses a different PI server
    start = "-1y"
    end = "*"
    step = "-0.1h"
    work_sheet = "DL_WORK"
