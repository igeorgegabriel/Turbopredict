...
def main() -> int:
    # ABF 21-K002 (C2 - Comp-Comp) configuration
    xlsx = PROJECT_ROOT / "excel" / "ABFSB" / "ABF LIMIT REVIEW (CURRENT).xlsx"
    tags_file = PROJECT_ROOT / "config" / "tags_abf_21k002.txt"
    safe_unit = "21-K002"  # canonical and filename-safe
    out_parquet = PROJECT_ROOT / "data" / "processed" / f"{safe_unit}_1y_0p1h.parquet"

    plant = "ABFSB"
    unit = "21-K002"  # canonical unit identifier (C2 compressor)

    # Flexible PI server configuration - check environment variable first
    server = os.getenv('ABF_PI_SERVER', r"\\VCENPOCEOPTNAP01\Protean\ABF\Compressor\21-K002")
    start = "-1y"
    end = "*"
    step = "-0.1h"
    work_sheet = "DL_WORK"
    ...
    alternative_servers = [
        r"\\PTSG-1MMPDPdb01",  # Default PCFS server
        r"\\ABF-PI-SERVER",
        r"\\ABF-PI-DB01",
        r"\\ABFSB-PI-SERVER",
        r"\\PTSG-ABF-DB01"
