#!/usr/bin/env python3
"""
Check PCMSB data freshness and investigate why units are not fetching new data.
"""
            ...
            try:
                ...
                if not df.empty:
                    ...
                    if data_age_hours > 24:
                        print(f'  ⚠️  STALE DATA: Over 24 hours old!')
                    elif data_age_hours > 6:
                        print(f'  ⚠️  OLD DATA: Over 6 hours old!')
                    else:
                        print(f'  ✅ FRESH DATA: Less than 6 hours old')
...
def check_pcmsb_build_scripts():
    ...
    for unit, script_path in pcmsb_scripts.items():
        ...
        if script_file.exists():
            print(f'{unit}: ✅ Build script exists - {script_path}')
            ...
            try:
                with open(script_file, 'r') as f:
                    ...
                    if 'PCMSB_Automation_Master.xlsx' in content:
                        print(f'       ✅ Correctly configured for PCMSB Excel')
                    else:
                        print(f'       ⚠️  May not be configured for PCMSB Excel')
            except Exception as e:
                print(f'       ❌ Error reading script: {e}')
                
        else:
            print(f'{unit}: ❌ Build script missing - {script_path}')
...
def check_pcmsb_excel_files():
    ...
    if excel_path.exists():
        print(f'✅ PCMSB Excel file exists: {excel_path}')
        ...
        try:
            ...
            if 'DL_WORK' in sheets:
                print(f'   ✅ DL_WORK sheet present')
            else:
                print(f'   ⚠️  DL_WORK sheet missing')
                
        except Exception as e:
            print(f'   ❌ Error reading Excel file: {e}')
            
    else:
        print(f'❌ PCMSB Excel file missing: {excel_path}')
