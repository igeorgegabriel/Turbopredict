"""Comprehensive parquet index check for all units"""
import pandas as pd
...
for plant in ['PCFS', 'ABF', 'PCMSB', 'ABFSB']:
    ...
    if plant_path.exists():
        units = sorted([d.name.replace('unit=', '') for d in plant_path.iterdir()
                       if d.is_dir() and d.name.startswith('unit=') and not 'OLD' in d.name])
        if units:
            print(f'\n  {plant}:')
            for unit in units:
...
for f in glob('data/processed/*_1y_0p1h.parquet'):
    if 'OLD' not in f and 'FIXED' not in f and 'archive' not in f:
        fname = Path(f).name
